    ...
    <AdminLayout>
        <div>
            ...
            <div class="mt-6 text-sm" style="color: var(--color-text-secondary)">
                Total Certificates: <span style="color: var(--color-text-primary); font-semibold;">{{ certificates.length }}</span>
            </div>
        </div>
    </AdminLayout>
</template>
    ...
    methods: {
        deleteCertificate(certId) {
            if (confirm('Are you sure?')) {
                this.$inertia.delete(route('admin.certificates.destroy', certId));
            }
        }
    }
}
