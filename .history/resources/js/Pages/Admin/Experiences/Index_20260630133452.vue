        ...
        <div>
            ...
            <div class="rounded-lg overflow-hidden"
                ...
                <table v-else class="w-full">
                    <thead style="border-bottom: 1px solid rgba(148, 163, 184, 0.1); background: rgba(148, 163, 184, 0.05);">
                        <tr>
                            <th class="px-6 py-4 text-left text-sm font-semibold" style="color: var(--color-text-primary)">
                                Title
                            </th>
                            <th class="px-6 py-4 text-left text-sm font-semibold" style="color: var(--color-text-primary)">
                                Company
                            ...
                            <th class="px-6 py-4 text-center text-sm font-semibold" style="color: var(--color-text-primary)">
                                Actions
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="exp in experiences" :key="exp.id"
                            ...
                            <td class="px-6 py-4 text-sm" style="color: var(--color-text-primary)">
                                <div class="font-medium">{{ exp.title }}</div>
                                <span v-if="exp.is_current" class="text-xs" style="color: var(--color-accent);">
                                    Currently working
                                </span>
                            </td>
                            <td class="px-6 py-4 text-sm" style="color: var(--color-text-secondary)">
                                {{ exp.company }}
                            </td>
                            <td class="px-6 py-4 text-sm" style="color: var(--color-text-secondary)">
            ...
            <div class="mt-6 text-sm" style="color: var(--color-text-secondary)">
                Total Experiences: <span style="color: var(--color-text-primary); font-semibold;">{{ experiences.length }}</span>
            </div>
        </div>
    </AdminLayout>
</template>
